...
{it:features} 
{ifin},
{cmd:mlmodel}{cmd:(}{it:{help c_ml_stata_cv##modeltype:modeltype}}{cmd:)}
{cmd:data_test}{cmd:(}{it:filename}{cmd:)}
{cmd:seed}{cmd:(}{it:integer}{cmd:)}
{cmd:[}{it:{help c_ml_stata_cv##learner_options:learner_options}}{cmd:}
{cmd:}{it:{help c_ml_stata_cv##cv_options:cv_options}}{cmd:}
{cmd:}{it:{help c_ml_stata_cv##other_options:other_options}}{cmd:]}


{dlgtab:Inputs}

{phang} {it:outcome}: categorical (integer) variable representing different classes    
...
{pstd} {cmd:c_ml_stata_cv} is a command for implementing machine learning classification algorithms in Stata 16.
...
support vector machine, standard (unregularized) multinomial. It provides hyper-parameters' optimal tuning via K-fold cross-validation using greed search.  
...
{syntab:Regularized multinomial}
...
{synopt:{opt prediction(name)}}Generating predictions of the outcome variable of name {it:name}. Both class and probability predictions are generated{p_end}
...
{syntab:Neural network}
{syntab:Scalars}
{synopt:{cmd:e(OOPT_NEURONS_L_1)}}Optimal number of neurons (or hidden units) in the first (hidden) layer{p_end}
{synopt:{cmd:e(OOPT_NEURONS_L_2)}}Optimal number of neurons (or hidden units) in the second (hidden) layer{p_end}
...
{phang3} {stata use iris_train, clear}{p_end}
{phang2} Run tree regression{p_end}
{phang3} {stata c_ml_stata_cv iris seplen sepwid petlen petwid , mlmodel("tree") data_test("iris_test") tree_depth(2 3 4 5 6 7 8) prediction("pred") cross_validation("CV") n_folds(5) seed(10)}{p_end}   
...
{phang}
Droste, M. 2022. Stata-pylearn. Available at: https://github.com/mdroste/statapylearn.
